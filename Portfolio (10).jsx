...
import {
  ...
  Brain, Filter, Star, ZoomIn, FileText, Eye, Shield, Upload, CheckCircle
} from "lucide-react";
...
// ═══════════════════════════════════════════════════════════════
// PATENT MODAL — shows actual PDF certificate
// ═══════════════════════════════════════════════════════════════
function PatentModal({ patent, dark, onClose, uploadedPdfUrl }) {
  const BG     = dark ? "rgba(2,6,23,0.97)"     : "rgba(248,250,252,0.97)";
  const BORDER = dark ? "rgba(255,255,255,0.1)"  : "rgba(0,0,0,0.1)";
  const TEXT   = dark ? "#f1f5f9"                : "#0f172a";
  const MUTED  = dark ? "#64748b"                : "#94a3b8";
          ...
          {uploadedPdfUrl ? (
            <div className="w-full h-full" style={{ minHeight: "500px" }}>
              <iframe
                ...
                style={{ height: "620px", border: `1px solid ${accent}30`, background: "#fff" }}
                title="UK Design Certificate"
              />
              <p className="text-center text-xs mt-3" style={{ color: MUTED }}>
                Scroll inside the viewer to see all pages · Press Esc to close
              </p>
            </div>
          ) : (
            // No PDF uploaded — show structured info instead
            <div className="space-y-4">
              ...
              {/* Upload hint */}
              <div className="rounded-xl p-4 text-center" style={{ background: "rgba(14,165,233,0.06)", border: `1px dashed ${accent}40` }}>
                <Upload className="w-6 h-6 mx-auto mb-2" style={{ color: accent, opacity: 0.6 }} />
                <p className="text-xs" style={{ color: MUTED }}>
                  Upload your PDF certificate using the <strong style={{ color: accent }}>Upload Certificate</strong> button on the patents card to display it here.
                </p>
              </div>
            </div>
          )}
...
export default function Portfolio() {
  ...
  // ── Patent PDF upload state ──
  const [patentPdfUrl, setPatentPdfUrl] = useState(null);   // blob URL of uploaded PDF
  ...
  useEffect(() => {
    return () => { if (patentPdfUrl) URL.revokeObjectURL(patentPdfUrl); };
  }, [patentPdfUrl]);
  ...
  const handlePdfUpload = (e) => {
    ...
    if (patentPdfUrl) URL.revokeObjectURL(patentPdfUrl);
    const url = URL.createObjectURL(file);
    setPatentPdfUrl(url);
    setPdfFileName(file.name);
    setTimeout(() => setPdfUploading(false), 600);
                  ...
                  <div className="flex flex-col md:flex-row gap-8 items-start">
                    ...
                    <div className="flex-1 space-y-6">
                      ...
                      <div className="flex flex-wrap gap-3 items-center">
                        {/* View Certificate (opens modal with PDF) */}
                        <motion.button whileHover={{ scale: 1.04, y: -2 }} whileTap={{ scale: 0.97 }}
                          ...
                          {patentPdfUrl ? "View Certificate PDF" : "View Full Details"}
                        </motion.button>
                        ...
                        <motion.button
                          ...
                          style={{
                            background: patentPdfUrl
                              ? "rgba(34,197,94,0.12)"
                              : dark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.04)",
                            border: patentPdfUrl
                              ? "1px solid rgba(34,197,94,0.4)"
                              : "1px solid rgba(14,165,233,0.3)",
                            color: patentPdfUrl ? "#22c55e" : "#0EA5E9",
                          ...
                          {patentPdfUrl ? "PDF Uploaded ✓" : "Upload Certificate PDF"}
                        </motion.button>

                        {/* Show filename if uploaded */}
                        {pdfFileName && (
                          <span className="text-xs px-3 py-1.5 rounded-full" style={{ color: MUTED, background: CARD_BG, border: `0.5px solid ${CARD_BORDER}` }}>
                            📄 {pdfFileName}
                          </span>
                        )}
                      </div>
                      ...
                      {/* Upload hint text */}
                      {!patentPdfUrl && (
                        <p className="text-xs" style={{ color: MUTED, opacity: 0.7 }}>
                          💡 Upload your UK design certificate PDF to display it inside the modal viewer.
                        </p>
                      )}
                    </div>
                    ...
                    <div className="flex-shrink-0 flex flex-col items-center justify-center gap-4 md:w-64">
                      ...
                      {/* PDF status indicator on shield side */}
                      {patentPdfUrl && (
                        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
                          className="flex items-center gap-2 px-4 py-2 rounded-full"
                          style={{ background: "rgba(14,165,233,0.1)", border: "1px solid rgba(14,165,233,0.3)" }}>
                          <FileText className="w-3.5 h-3.5" style={{ color: "#0EA5E9" }} />
                          <span className="text-xs font-bold" style={{ color: "#0EA5E9" }}>Certificate Loaded</span>
                        </motion.div>
                      )}
                    </div>
                  </div>
