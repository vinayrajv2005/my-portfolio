...
const projects = [
  ...
  },
  {
    ...
    desc: "Replace with your real project description — the problem it solves, the model used, and key results or accuracy metrics.",
    tags: ["Web", "React", "API"],
...
// ═══════════════════════════════════════════════════════════════
// CERTIFICATE MODAL — original full version preserved
// ═══════════════════════════════════════════════════════════════
            ...
            <div className="flex flex-wrap gap-2">
              {patent.inventors.map((inv, i) => (
                <span key={i} className="px-3 py-1.5 rounded-full text-xs font-semibold"
                  style={{
                    background: inv.includes("Vinay") ? accent + "25" : CARD,
                    color: inv.includes("Vinay") ? accent : MUTED,
                    border: `0.5px solid ${inv.includes("Vinay") ? accent + "60" : BORDER}`,
...
// ═══════════════════════════════════════════════════════════════
// PHOTO LIGHTBOX — original fully preserved
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// CUSTOM CURSOR — original fully preserved
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// TYPING HOOK — original fully preserved
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// SKILL CATEGORIES GRID — original fully preserved
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// CONTACT FORM — original fully preserved (with setLoading fix)
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// AI CHATBOT — original fully preserved + patent quick reply
// ═══════════════════════════════════════════════════════════════
...
export default function Portfolio() {
  ...
  const [sound, setSound] = useState(false);           // ✅ RESTORED
  const [filter, setFilter] = useState("All");
  const [showTop, setShowTop] = useState(false);
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [selectedCert, setSelectedCert] = useState(null);
  const [selectedPatent, setSelectedPatent] = useState(null); // ✅ NEW
  const [photoOpen, setPhotoOpen] = useState(false);   // ✅ RESTORED
    ...
    <div style={{ background: BG, color: TEXT, cursor: "none", fontFamily: "'DM Sans','Nunito',sans-serif", minHeight: "100vh", overflowX: "hidden" }}>
      ...
      <Section id="home" className="min-h-screen flex flex-col justify-center items-center text-center px-6 pt-20">
        <motion.div initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          ...
          {/* ✅ RESTORED: Clickable photo with ZoomIn hover overlay */}
          <motion.div
                    ...
                    <div className="flex items-start justify-between mb-5">
                      ...
                      <div className="flex items-center gap-1 text-xs" style={{ color: MUTED }}>
                        ...
                        <span className="ml-2 px-2 py-0.5 rounded-full text-xs"
                          style={{ background: CARD_BG, border: `0.5px solid ${CARD_BORDER}` }}>{p.year}</span>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col md:flex-row gap-8 items-start">
                    ...
                    <div className="flex-1 space-y-6">
                      ...
                      <div className="flex flex-wrap gap-3">
                        ...
                        <motion.a href="https://www.gov.uk/ipo" target="_blank" rel="noopener noreferrer"
                          whileHover={{ scale: 1.04, y: -2 }} whileTap={{ scale: 0.97 }}
                          className="flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold"
                          style={{ background: CARD_BG, border: "1px solid rgba(14,165,233,0.3)", color: "#0EA5E9" }}>
